...
const DeliveryAvailable = (props: any) => {
  ...
  areaDataGroup.forEach((val: any, key: any, mapObject: any) => {
    if (val[0].PAY_AMT === 0 && val.length > 1) {
  ...
  const onSelectDelete = () => {
    if (gridApi != null) {
      ...
      if (checkData.length === 0) {
        setAlertText('삭제할 배송가능 구역을 선택하세요.');
        setOpenA((o: any) => !o);
        return;
      }
      ...
      confirmAlert({
        title: '삭제여부 확인',
        message: `총 ${checkData.length}개의 배송가능 요금을 삭제하시겠습니까?`,
        buttons: [
          {
            label: 'YES',
            onClick: () => {
    ...
    <Split className="split" sizes={[17, 83]}>
      ...
      <div className=" left_box flex_section_column">
        <div className="flex_section_row">
          <div className="blueLabel mt05 ">지원센터</div>

          {!hcData && <input type="text" className="baseInput mg05" style={{ width: '140px' }} />}
          {hcData &&
            hcData.map((hc: HelperCompany) => (
              <input
                type="text"
        ...
        <div className="flex_section_row">
          <div className="blueLabel mt05">회원사</div>
          <select className="baseSelect mg05" style={{ width: '110px' }}>
            {mcData &&
              mcData.length > 0 &&
              mcData.map((mc: MemberCompany) => (
                <option key={mc.MCODE} value={mc.MNAME}>
                  {mc.MNAME}
                </option>
              ))}
          </select>
        ...
        <div className="flex_section_row">
          <Button
            ...
            size="small"
            className="mt05"
            style={{ width: '110px', color: '#fff', backgroundColor: '#3f51b5' }}
        ...
        <div
          className="flex_section_row mt05 stretch"
          style={{
            height: '100%',
          }}
        >
      ...
      <div className="right_box flex_section_column ml10">
        <div className="flex_section_row align_center mt05">
          <div className="blueText fontsize_13 mt10">
            ※ 구역설정방법 : 구역설정시작 클릭 -&gt; 구역정보입력(명칭) -&gt; 우측마우스 구역설정
            -&gt; 구역설정완료 클릭 <br />
            <br />※ 구역이 겹치는경우 먼저등록된 구역이 적용됩니다. ※ 구역리스트 더블클릭시
            구역정보삭제
          </div>
          <Button
            variant="contained"
